...
export default class GraphQLTableView<TItem, TVariables> extends React.Component<
    ...
    public async fetch() {
        ...
        result.subscribe(({ data, loading }) => {
            ...
            this.setState({
                datasource: parseSDLToTableColumnInfos(this.props.query, this.props.introspection, data[dataKey]),
                loading
            })
        })
    }
    ...
    public render() {
        return (
            <div>
                <table >
                    {this.state.datasource.length > 0 ? <GraphQLTableHeader column={this.state.datasource[0]} /> : null}
                    <tbody>
